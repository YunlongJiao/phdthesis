...
\begin{frame}{\insertsubsection}
	...
	\begin{itemize}
		...
		\begin{itemize}
			...
			\item[$\checkmark$]<5-> Network-guided gene selection.
			\only<5>{
			\begin{figure}
				\centering
...
\subsection{\scshape Positive definite kernels for permutations}
\begin{frame}{\insertsubsection}
	...
	\<{0.75\linewidth}
		\begin{itemize}
			\item<1-> \alert<1>{Kendall (tau) distance} \cite{Kendall1938new} counts the number of discordant pairs between permutations, i.e.,
			\begin{equation*}
			\begin{split}
			...
			\item<2-> \alert<2>{Kendall kernel} is defined by
			$$ K_\tau(\sigma, \sigma') := 1 - \frac{2d(\sigma, \sigma')}{{n\choose 2}} \,. $$
			
			\item<3-> \alert<3>{Mallows kernel} is defined for any $\lambda \geq 0$ by
	...
	\<{0.4\linewidth}
		...
		\uncover<3->{
		$$ K_M^\lambda (\sigma,\sigma') = e^{-2\lambda} \,, \lambda \geq 0 $$
		}
	\>
	\)
	}
	...
	\begin{thm}[Main theorem \cite{Jiao2015Kendall}]
		$K_\tau$ and $K_M^\lambda$ are positive definite, and $d$ is negative definite.
	\end{thm}
	}
	...
	\footnotesize
		...
		\begin{alignat*}{2}
			\mbox{Kendall kernel} &: \quad K_\tau(\sigma, \sigma') && = \phi(\sigma)^\top \phi(\sigma') \,, \\
			\mbox{Mallows kernel} &: \quad K_M^\lambda(\sigma, \sigma') && = e^{-\lambda \nm{\phi(\sigma) - \phi(\sigma')}^2} \,, \\
			\mbox{Kendall distance} &: \quad \quad d(\sigma, \sigma') && = \nm{\phi(\sigma) - \phi(\sigma')}^2 \,.
		\end{alignat*}
	\end{proof}
	}
	...
	\begin{thm}[Kernel trick \cite{Knight1966Computer}]
		$K_\tau$, $K_M^\lambda$ and $d$ can all be evaluated in \alert<6>{$O(n\log n)$} time.
	\end{thm}
	}
	...
	\<{0.65\linewidth}
	\begin{itemize}
		\item Diffusion kernel vs Mallows kernel on $\Sn$:
	\end{itemize}
	\>
	\hspace{-1cm}
	\<{0.4\linewidth}
	...
	\begin{tabular}{c|cc}
		...
		$K(\sigma,\sigma')$ & $\sqb{e^{-\lambda \Delta}}_{\sigma,\sigma'}$ & $e^{-\lambda d(\sigma, \sigma')}$ \\
...
\begin{frame}{\insertsubsection}
	...
	\<{0.5\linewidth}
		...
		\begin{itemize}
			...
			\item<3-4> \alert<3>{Less sensitive to regularization parameter!}
...
\begin{frame}{\insertsubsection}
	...
	\begin{thm}[Tractable approx bound \cite{Jiao2016Controlling}]
		For any approximation $\sigma$ such that the Euclidean angle $\theta(\sigma)$ between $\phi(\sigma)$ and $\phi(\DD)$ is less than $\frac{\pi}{2}$, then $d(\sigma, \sigstar) \leq \left\lfloor {n \choose 2} \sin^2(\theta(\sigma)) \right\rfloor \, .$
	\end{thm}
	}
...
\begin{frame}{\insertsubsection}
	...
	\begin{figure}
		...
		\caption{Voting blocs in Irish electorate \cite{Gormley2008Exploring}.}
	\end{figure}
	}
...
\begin{frame}{\insertsubsection}
	...
	\begin{itemize}
		...
		\begin{itemize}
		\item<1->[-] They reflect different level of global smoothness.
	...
	\centering
		...
		\caption{Graph Fourier basis $\mathbf{u}_1$ (left) and $\mathbf{u}_n$ (right) corresponding to the smallest and largest eigenvalue \cite{Rapaport2007Classification}.}
	\end{figure}
	}
	...
	\only<3-4>{
		...
		\only<3>{
			\begin{figure}
				\includegraphics[width=0.5\linewidth]{slides/sgwt}
				\caption{Scaled wavelet generating kernels $g(s_j \lambda)$ at different scales $s_j > 0$ \cite{Hammond2011Wavelets}.}
			\end{figure}
		}
		\only<4>{
		\begin{itemize}
	...
	\>
	\<{0.7\linewidth}
		\caption{Spectral graph wavelets around one vertex at different scales \cite{Hammond2011Wavelets}.}
	\>
	\)
	\end{figure}
	}

...
\begin{frame}{\insertsubsection}
	...
	\<{0.65\linewidth}
		\begin{figure}
			\only<4>{\includegraphics[width=\columnwidth]{ch-kendall/cluster_results/kmeans-time}}
			\only<5>{\includegraphics[width=\columnwidth]{ch-kendall/cluster_results/kmeans-silhouette}}
			\only<6>{\includegraphics[width=\columnwidth]{ch-kendall/cluster_results/kmeans-stability}}
		\end{figure} 
	\>
	\<{0.5\linewidth}
		Kendall kernel $k$-means has
		\begin{itemize}
			\item<4-6> \alert<4>{Less run time!}

...
The paper is organized as follows. In Section~\ref{sec2:kernel}, we prove our main theorem showing that the Kendall and Mallows kernels are positive definite. We extend them to partial, multivariate and uncertain rankings respectively in Section~\ref{sec2:partial}, \ref{sec2:multivariate} and~\ref{sec2:uncertain}. We highlight the relation to the diffusion kernel of \cite{Kondor2010Ranking} in Section~\ref{sec2:group}. Finally we illustrate the relevance of kernel methods for unsupervised (Section~\ref{sec2:clustering}) and supervised (Section~\ref{sec2:classification}) tasks. Data and \texttt{R} codes for generating all the plots in this paper and reproducing more experiments are available via \url{https://github.com/YunlongJiao/kendallkernel_demo}. I have also developed \texttt{kernrank}, an \texttt{R} package implementing kernel functions and kernel methods for analyzing rank data, publicly available at \href{https://github.com/YunlongJiao/kernrank}{https://github.com/YunlongJiao/kernrank}.
...
\begin{equation*}
	\label{eq2:kdtnc}
	\begin{split}
	n_c(\sigma,\sigma') & =  \sum_{i<j} \big[\mathds{1}_{\{\sigma(i)<\sigma(j)\}} \mathds{1}_{\{\sigma'(i)<\sigma'(j)\}}
	+ \mathds{1}_{\{\sigma(i)>\sigma(j)\}}\mathds{1}_{\{\sigma'(i)>\sigma'(j)\}}\big]\,, \\
	n_d(\sigma,\sigma') & =  \sum_{i<j} \big[\mathds{1}_{\{\sigma(i)<\sigma(j)\}} \mathds{1}_{\{\sigma'(i)>\sigma'(j)\}} 
	+ \mathds{1}_{\{\sigma(i)>\sigma(j)\}}\mathds{1}_{\{\sigma'(i)<\sigma'(j)\}}\big] \,.
\end{split}
\end{equation*}
...
\begin{proof}
Consider the mapping $\Phi:\Sn \rightarrow \RR^{{n \choose 2}}$ defined by
$$
\Phi(\sigma)=\bigg( \frac{1}{\sqrt{{n \choose 2}}} (\mathds{1}_{\{\sigma(i)>\sigma(j)\}}-\mathds{1}_{\{\sigma(i)<\sigma(j)\}}) \bigg)_{1\leq i < j \leq n} \,.
$$
...
\begin{proof}
The proof is constructive. We show here explicitly how to compute the Kendall kernel between two interleaving partial rankings while the idea remains similar for the case of top-$k$ partial rankings. Denote by $[1,n]$ the item set to be ranked and by $A_{i_1,\dots,i_k},A_{j_1,\dots,j_m}\subset \mathbb{S}_n$ two interleaving partial rankings of size $k,m$ respectively, whose subsets of item indices are denoted by $I:=\{i_1,\dots,i_k\}$ and $J:=\{j_1,\dots,j_m\}$. We will lighten the notation by writing $A_I := A_{i_1,\dots,i_k}$ and $A_J := A_{j_1,\dots,j_m}$ and recall that by definition,
\begin{equation*}
\begin{split}
...
\end{multline}
As we will always regard the item set $[1,n]$ as the universe, we will write the complement of set $S \subset [1,n]$ as $S^\complement := [1,n]\setminus S$. Since the item set can be divided into four disjoint subsets that are $[1,n] = (I \cap J) \sqcup (I \setminus J) \sqcup (J \setminus I) \sqcup (I \cup J)^\complement$, any (unordered) item pair $\{i,j\}$ can be categorized uniquely into one out of ten cases:
\begin{itemize}
...
\begin{equation}
	...
	\Phi(\xb) = \bigg( \frac{1}{\sqrt{{n \choose 2}}} (\mathds{1}_{\{x_i>x_j\}}-\mathds{1}_{\{x_i<x_j\}}) \bigg)_{1\leq i < j \leq n}\,.
\end{equation}
...
\begin{figure}[!htbp]
\begin{center}
\includegraphics[width=0.6\textwidth]{ch2kendall/figures/smoothheaviside}
...
\begin{figure}[!htbp]
\begin{center}
\centerline{\includegraphics[width=0.6\textwidth]{ch2kendall/figures/permutahedron}}
...
\begin{figure}[!htbp]
\centering
\includegraphics[width=0.6\textwidth]{ch2kendall/cluster_results/kmeans-time}\caption{Computational time (in seconds) of $k$-means algorithms per run across different number of clusters.}
\label{fig2:kmeans-time}
\end{figure}

\begin{figure}[!htbp]
\centering
\includegraphics[width=0.6\textwidth]{ch2kendall/cluster_results/kmeans-silhouette}
...
\begin{figure}[!htbp]
\begin{center}
\includegraphics[width=0.6\textwidth]{ch2kendall/cluster_results/kmeans-stability}
...
\begin{figure}[!htbp]
\centering
\includegraphics[width=0.6\textwidth]{ch2kendall/cluster_results/mallows-silhouette}
...
\centering
\includegraphics[width=0.45\textwidth]{ch2kendall/cluster_results/eurovision-kkmeans}%
\includegraphics[width=0.45\textwidth]{ch2kendall/cluster_results/eurovision-silhouette-kkmeans}
...
\begin{subfigure}{\textwidth}
\centering
\includegraphics[width=0.45\textwidth]{ch2kendall/cluster_results/eurovision-rankclust}%
\includegraphics[width=0.45\textwidth]{ch2kendall/cluster_results/eurovision-silhouette-rankclust}
...
\includegraphics[width=0.6\textwidth]{ch2kendall/clasf_results/acc_plot}
\
\end{center}
...
\includegraphics[width=0.6\textwidth]{ch2kendall/clasf_results/acc_perfSVM}
...
\includegraphics[width=0.6\textwidth]{ch2kendall/clasf_results/acc_perfFS}
...
\begin{figure}[!htbp]
\begin{center}
\includegraphics[width=0.6\textwidth]{ch2kendall/clasf_results/acc_StabApprox}
\end{center}
...
\begin{figure}[!htbp]
\begin{center}
\includegraphics[width=0.6\textwidth]{ch2kendall/clasf_results/acc_approxSingleWindow}
\end{center}

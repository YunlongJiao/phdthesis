...
The work in this chapter has been published as joint work with Anna Korba and Eric Sibony in \cite{Jiao2016Controlling} and orally presented at ICML 2016 by Anna Korba.
\end{chapabstract}
\vskip 0.2in
...
A final lemma necessary for the proof of Theorem \ref{thmA:method} is on the embedding of a ball in the Euclidean space. For $\sigma\in\Sn$ and $k\in\{0,\dots,\binom{n}{2}\}$, we denote by $B(\sigma,k)$ the (closed) ball for the Kendall's tau distance of center $\sigma$ and radius $k$, i.e. $B(\sigma,k) = \{\sigma'\in\Sn \;\vert\; d(\sigma,\sigma')\leq k\}$. The following is a direct consequence of Proposition \ref{propA:background-result}.
